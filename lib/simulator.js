...
function Simulator(scene) {
  ...
  this.dist = 0;
  this.all = [];
  this.cur = new THREE.Vector3(0,0,0);
  this.all = [this.cur];
  this.box = {
    xmin: 0,
    xmax:0,
...
Simulator.prototype = {
  parseMeta: function(gcode) {
    var attrs = gcode.match(/([a-z]+)\s*=\s*([^\s\)]+)/g)
    ...
    attrs.forEach(function(attr) {
      ...
      var k = kv[0].trim();
      var v = kv[1].trim();
      var type = this.metaTypes[k.toLowerCase()] || String;
      this.meta[k] = type(v);
    }, this);
...
, addPoint: function(p) {
    ...
    this.zmin = Math.min(this.z, this.zmin);
    this.xmax = Math.max(this.x, this.xmax);
    this.ymax = Math.max(this.y, this.ymax);
    this.zmax = Math.max(this.z, this.zmax);
